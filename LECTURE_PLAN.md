...
**Corpus:** `makani-docs/` — 16 documents → **71 indexed sections**, 6 components,
2 superseded docs, dates spanning 2024–2026.

**Shared plumbing:** `makani.py` (loading, `##`-section splitting, metadata parsing,
...
*"Makani won't start after last night's deploy and the logs mention something it needed
...
`makani.config.yaml` or the environment, restart Sapir."* The current procedure.
...
Keys read from a local `.env` via `makani.api_key()`; override with `MAKANI_ENV_FILE`.
...
**This is the etesian notebook.** It replicates `server/search/llm/question_to_sql.py`:
a **small** model (`gpt-4.1-mini`, etesian's `ModelTier.SMALL`) translates the question into
...
- `chain_of_thought` is the **first property**, so the model reasons before committing to
  values (etesian: `# Order matters here, the model fills fields top-down`).
- `query_type` (`content_lookup` / `corpus_enumeration` / `aggregate`) gates `keywords`.
  Without it the model emitted `keywords: ["Makani startup failure"]` for a pure
...
each answer. **We write the reasoning; the model copies its shape.** Etesian ships 36; we
...
DISTINCT filename because chunks are sections. Etesian's equivalents: *"do NOT use CTEs"*,
*"do NOT use `IN (...)` with `lookup_user_by_name_or_email`"*.
...
Honest weak spot: latency — 2 sequential calls (~9s) vs one (~5.5s). Etesian runs its
...
(etesian discards the model's FTS terms entirely and substitutes its own).
...
**Correction worth knowing:** etesian has **no `PART_OF` / `RELATES_TO` enum.** Edge labels are
strings *derived* by `get_edge_type(src_type, dst_type, label)` in `ingest/edge_types.py`, so a
...
keyed on the label reproduces etesian's `UNIQUE(source, target, label)` index.

**§2 Traverse from the retrieval seeds** — the architectural point: **the graph does not replace
retrieval, it expands it.** etesian seeds `traverse_from_seeds()` with the top-250 BM25 hits then
...
(etesian does this for `person`).

Result: **3 retrieved docs → 20 connected nodes**, including `incident-2026-03-config-outage.md`
and `makani-architecture-overview.md`, which match no query term and arrive purely by connection.
...
`doc → Sapir → {Kesh, Nugat} → {Vello, Lomi}` runs out one hop short. etesian's traversal is built
...
matches the question, and production runs all four, which is exactly etesian: BM25 seeds → graph
