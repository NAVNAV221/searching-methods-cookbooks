...
> file to `makani.config.yaml` and the remote config store. The migration is
...
`makani.config.yaml`, the process environment, or the remote config store.
...
1. Run `sapir migrate-conf` to emit a `makani.config.yaml` from the existing
   `sapir.conf`.
