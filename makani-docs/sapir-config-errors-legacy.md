...
Sapir is the service that loads Makani's configuration at startup. This
page documents `CONFIGURATION_IS_MISSING`, the error Makani operators most
often ask about, as it worked on older Makani releases.

## CONFIGURATION_IS_MISSING

Sapir logs `CONFIGURATION_IS_MISSING` when a required configuration key
for Makani is missing from `sapir.conf`. Makani will not start while this
...
3. Run `sapir --reload-config` (this flag was removed in the 2025
   migration — use a full restart instead on current Makani releases).
4. If `CONFIGURATION_IS_MISSING` persists after restart, page
   platform-team directly about the Makani configuration issue, since the
